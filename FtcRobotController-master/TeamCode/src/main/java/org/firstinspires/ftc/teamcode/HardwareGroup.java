...
public class HardwareGroup {
    public class Group<T extends HardwareDevice> {
        public T[] hardware;
        private T firstObj;
        ...
        public void setPower(double d) {
            ...
            else if (firstObj instanceof DcMotorSimple) for (T obj : hardware) ((DcMotorSimple) obj).setPower(d);
        ...
        public void setTargetPosition(int p) {
            if (firstObj instanceof DcMotorEx) for (T obj : hardware) ((DcMotorEx) obj).setTargetPosition(p);
            else if (firstObj instanceof DcMotor) for (T obj : hardware) ((DcMotorEx) obj).setTargetPosition(p);
            else unsupported("DcMotor");
        }
        ...
        public void setDirection(DcMotor.Direction dir) {
            ...
            else if (firstObj instanceof DcMotor) for (T obj : hardware) ((DcMotorEx) obj).setDirection(dir);
            else if (firstObj instanceof DcMotorSimple) for (T obj : hardware) ((DcMotorSimple) obj).setDirection(dir);
            else unsupported("DcMotorSimple");
        }

        public void setZeroPowerBehavior(DcMotor.ZeroPowerBehavior zpb) {
            if (firstObj instanceof DcMotorEx) for (T obj : hardware) ((DcMotorEx) obj).setZeroPowerBehavior(zpb);
            else if (firstObj instanceof DcMotor) for (T obj : hardware) ((DcMotorEx) obj).setZeroPowerBehavior(zpb);
            else unsupported("DcMotor");
        }
        ...
        public boolean[] isPressed() {
            if (firstObj instanceof TouchSensor) {
                ...
                for (int i = 0; i < hardware.length; i++) results[i] = ((TouchSensor) hardware[i]).isPressed();
                return results;
